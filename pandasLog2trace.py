...
def check_and_parse_args():
    ...
    parser.add_argument('-r', '--runindex', type = int, default = -1,
                        help = "Index of run, -1 is latest")
...
def main():
    ...
    for pid, (participant, f) in enumerate(args.logs.items()):
        ...
        if args.noglobal: df = df[df.Name != "_GLOBAL"]

        if args.ranks: df = df[df['Rank'].isin(args.ranks)]
        ...
        for index, row in df.iterrows():
            ...
            event = {
                ...
                "tid": rank,
                "pid": pid,
                "ts": row["Timestamp"] * 1000, # convert from ms to µs
            }
